...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_trip'])) {
    ...
    // Insert Query
    $sql = "INSERT INTO nh_register (vehicle_no, quantity, date, time, op_code, distance, direct_count, indirect_count, done) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    
    if ($stmt) {
        $stmt->bind_param('sisssddii', $vehicle_no, $quantity, $date, $time, $op_code, $distance, $direct_count, $indirect_count, $done);
