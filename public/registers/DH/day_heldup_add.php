...
// We join 'reason' table with 'gl' table on gl_code to get gl_name (which acts as the category)
$all_reasons_sql = "
    SELECT 
        r.reason_code, 
        r.reason, 
...
if ($op_code_result) {
    while ($row = $op_code_result->fetch_assoc()) {
        // We only need the code here, vehicle fetching is done via AJAX
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="w-[85%] ml-[15%] flex justify-center p-3">
        <div class="container max-w-2xl bg-white shadow-lg rounded-lg p-8 mt-2">
            ...
            <h1 class="text-3xl font-extrabold text-gray-900 mb-2 border-b pb-2">
                Add New Day Heldup Trip
            </h1>
            <p class="text-sm text-gray-600 mb-4">
                Use this form to manually log a new trip that experienced a delay or heldup incident.
            </p>
            ...
            <form id="addHeldupForm" class="space-y-6">
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="time" id="out_time" name="out_time" required value="" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2">
                    </div>
                    <div>
                        <label for="in_time" class="block text-sm font-medium text-gray-700">In Time<span class="text-red-500">*</span></label>
                        <input type="time" id="in_time" name="in_time" required value="" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2">
                    </div>
                </div>

                <div>
                    <label for="distance" class="block text-sm font-medium text-gray-700">Distance (km)</label>
                    <input type="number" step="0.1" min="0" id="distance" name="distance" placeholder="Enter distance (Optional)" value="" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2">
                </div>
                        ...
                        <div class="md:col-span-2 relative">
                            ...
                            <select id="reason_code_0" name="reason_code[]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2">
                                ...
                                foreach ($available_reasons as $reason):
                                    // reason_category is now gl_name
    ...
    <script>
        ...
        let entryCounter = 1;
        // The structure of availableReasons array now contains gl_name under the 'reason_category' key
        ...
        // Creates a new Reason Dropdown using the PHP-fetched data (which is now categorized by gl_name)
        ...
        opCodeSelect.addEventListener('change', async function() {
            ...
            try {
                // Assuming 'day_heldup_fetch_details.php' fetches vehicle_no based on op_code
        ...
        addHeldupForm.addEventListener('submit', async function(event) {
            ...
            if (!loggedInUserId) {
                 showToast('Error: User session is missing. Please log in again.', 'error');
                 return;
            }
            ...
            if (!valid || reasonData.length === 0) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-save mr-1"></i> Submit';
                return;
            }
            
            const formData = new FormData(this);
            // Append processed JSON reason data instead of the raw array
            formData.set('reason_data_json', JSON.stringify(reasonData));
            // Remove raw arrays to prevent PHP mixing them up
            ...
            try {
                // Submit to a new dedicated handler
                const response = await fetch('day_heldup_add_handler.php', { 
                    method: 'POST',
                    body: new URLSearchParams(formData),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                });
