<?php
// dh_attendance_mark.php - Form to mark daily Op Code / Vehicle Attendance (Auto Date/Time Input)
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="w-[85%] ml-[15%] flex justify-center p-6">
        <div class="container max-w-2xl bg-white shadow-lg rounded-lg p-8 mt-6">
            ...
            <h1 class="text-3xl font-extrabold text-gray-900 mb-2 border-b pb-3">
                Mark Daily DH Attendance
            </h1>
            <p class="text-sm text-gray-600 mb-6">
                Date and Time will be recorded automatically at the time of submission.
            </p>
    ...
    <script>
        ...
        // --- FETCH Vehicle No Logic (from day_heldup_fetch_details.php) ---
        ...
        document.getElementById('attendanceForm').addEventListener('submit', async function(event) {
            ...
            try {
                // Submit to the handler (which will generate date/time internally)
                const response = await fetch('dh_attendance_handler.php', { 
                    method: 'POST',
                    body: new URLSearchParams(formData),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                });
