...
// Construct the base SQL query, using the worker table
$sql = "SELECT
            ed.date,
            ed.vehicle_no,
            r.route AS route_name,
            wtv.driver AS driver,
            ed.distance AS extra_distance,
            ed.remark AS reason
        FROM
            worker_extra_distance ed
        LEFT JOIN
            route r ON ed.route_code = r.route_code
        LEFT JOIN
            worker_transport_vehicle_register wtv ON ed.date = wtv.date
            AND ed.route_code = wtv.route
            AND wtv.shift = 'evening'";
...
<head>
    <meta charset="UTF-8">
    <title>Extra Distance Details (Workers)</title>
...
<div class="bg-gray-800 text-white p-2 flex justify-between items-center shadow-lg w-[85%] ml-[15%]">
    ...
    <div class="flex gap-4">
        <a href="absent_staff.php" class="hover:text-yellow-600">Absent</a>
        <a href="add_staff_record.php" class="hover:text-yellow-600">Add Record</a>
        <a href="extra_distance.php" class="hover:text-yellow-600">Add Extra Distance</a>
        <a href="" class="text-yellow-600">View Extra Distance</a>
        <a href="barcode_reader.php" class="hover:text-yellow-600">Barcode</a>
    </div>
</div>
...
<div class="container" style="width: 80%; margin-left: 18%; margin-right: 2.5%; display: flex; flex-direction: column; align-items: center;">
    ...
    <div class="w-full flex justify-between items-center mt-4">
        ...
        <p class="text-[48px] font-bold text-gray-800 text-center">Extra Distance Details (Workers)</p>
        <div class="flex-grow flex justify-end">
            <a href="view_extra_distance.php" class="bg-[oklch(25.7%_0.09_281.288)] text-white px-4 py-2 rounded-md hover:bg-purple-900">
                Show Staff
            </a>
        </div>
    </div>
                ...
                <?php else : ?>
                    <?php foreach ($extra_distance_data as $row) : ?>
                        <tr>
                            ...
                            <td class='border px-4 py-2'><?php echo htmlspecialchars($row['driver']); ?></td>
