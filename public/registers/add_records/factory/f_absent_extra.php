...
$factory_transport_gl_code = '623401';
...
function getRouteCodes($conn): array {
    ...
    $sql = "SELECT route_code, route FROM route WHERE purpose='factory' ORDER BY route_code ASC"; 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $check_sql = "
            ...
            SELECT 'factory_transport' AS source, route AS route_id
        ...
        if ($check_result->num_rows > 0) {
            ...
            $source = ($row['source'] == 'factory_transport') ? 'Primary Vehicle Register' : (($row['source'] == 'extra_vehicle') ? 'Extra Vehicle Register' : 'Petty Cash Register');
    ...
    try {
        ...
        if ($rate_row) {
            ...
            throw new Exception("Route configuration data missing.");
        }
        
        if (empty($route_supplier_code)) {
             throw new Exception("The primary route supplier code is missing.");
        }
        ...
        $final_message = "Extra vehicle record added.";
        
        if ($net_adjustment > 0) {
             $final_message .= "Contractor payment adjusted.";
        } elseif ($amount > $trip_rate) {
             $final_message .= "Contractor adjustment skipped.";
             $status = "warning";
        } else {
        ...
        if ($cost_allocated) {
            $final_message .= " Cost successfully allocated to departments.";
    ...
    if ($success || $status == 'error') {
        ...
        $register_url = BASE_URL . "registers/factory_transport_vehicle_register.php";
