...
// මෙය HTML සබැඳියෙන් ලැබෙන YYYY-MM ආකෘතියේ අගයයි.
...
// Fetch all routes (using the staff route pattern: ____S%)
$routes_data = [];
$sql_routes = "SELECT route_code, route FROM route WHERE route_code LIKE '____S%' ORDER BY route_code ASC";
...
foreach ($routes_data as $route) {
    ...
    // Query trip details (using staff_transport_vehicle_register table)
    $sql_details = "
        SELECT 
