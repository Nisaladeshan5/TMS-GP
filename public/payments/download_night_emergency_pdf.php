<?php
// download_night_emergency_pdf.php - Generates PDF summary for Night Emergency (Op Code wise)
...
$op_code = isset($_GET['op_code']) ? $_GET['op_code'] : '';

if (empty($op_code)) {
    die("Op Code missing.");
}

// Calculate Data for specific Op Code
$sql = "
    SELECT 
        s.supplier,
        s.supplier_code,
...
$stmt->bind_param("sii", $op_code, $selected_month, $selected_year);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows == 0) {
    die("No records found for Op Code: $op_code in this period.");
}
...
$pdf->Cell(0,5,"Op Code: " . $op_code . " (Vehicle: " . $data['vehicle_no'] . ")",0,1,'C');
$pdf->Ln(8);
...
// Payment Summary Table (Updated Layout)
$pdf->SetFont('Arial','B',10);
$pdf->SetFillColor(220,220,220); // Gray Header
// 2 Column Layout: 110 + 80 = 190
$pdf->Cell(110,8,'Description',1,0,'L',1);
$pdf->Cell(80,8,'Amount / Value',1,1,'R',1);

$pdf->SetFont('Arial','',10);
$pdf->SetFillColor(255,255,255);

// Worked Days
$pdf->Cell(110,8,'Total Worked Days',1,0,'L',1);
$pdf->Cell(80,8,$data['worked_days'],1,1,'R',1);

// Rate
$pdf->Cell(110,8,'Rate Per Day (LKR)',1,0,'L',1);
$pdf->Cell(80,8, number_format($data['day_rate'], 2),1,1,'R',1);

// Final Total
$pdf->SetFont('Arial','B',11);
$pdf->SetFillColor(220,220,220);
$pdf->Cell(110,10,'TOTAL PAYMENT (LKR)',1,0,'L',1);
$pdf->Cell(80,10,number_format($total_payment,2),1,1,'R',1);

$pdf->Ln(10);

// Output PDF
$filename = "NE_Report_".$op_code."_".date('Y-m', mktime(0,0,0,$selected_month,1,$selected_year)).".pdf";
