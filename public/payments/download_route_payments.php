...
if ($routes_result && $routes_result->num_rows > 0) {
    while ($route_row = $routes_result->fetch_assoc()) {
        ...
        $suppliers_sql = "
            SELECT DISTINCT stvr.supplier_code 
        ...
        if ($suppliers_result->num_rows > 0) {
            ...
            while ($supplier_row = $suppliers_result->fetch_assoc()) {
                ...
                $vehicle_lookup_sql = "SELECT vehicle_no FROM staff_transport_vehicle_register WHERE route = ? AND supplier_code = ? AND MONTH(date) = ? AND YEAR(date) = ? LIMIT 1";
