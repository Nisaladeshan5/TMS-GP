...
function calculate_total_payment($conn, $route_code, $supplier_code, $month, $year, $route_distance, $fixed_amount, $with_fuel, $consumption_id, $rate_id, $consumption_rates, $default_km_per_liter)
{
    ...
    foreach ($daily_trip_counts as $daily_data) {
        ...
        if ($with_fuel === 1 && $consumption_id !== null && $latest_fuel_price > 0 && $km_per_liter > 0) {
...
function get_total_adjustment_amount($conn, $route_code, $supplier_code, $month, $year)
{
    ...
    if ($reduction_stmt->execute()) {
        ...
        return (float)($row['total_adjustment_amount'] ?? 0);
    }
    return 0.00;
}
...
// --- MAIN DATA FETCH ---
$payments_sql = "
    SELECT DISTINCT stvr.route AS route_code, stvr.supplier_code, r.route, r.fixed_amount, r.distance AS route_distance, r.with_fuel, v.fuel_efficiency, v.rate_id
...
if ($payments_result && $payments_result->num_rows > 0) {
    while ($payment_row = $payments_result->fetch_assoc()) {
        $calculation_results = calculate_total_payment(
            $conn, $payment_row['route_code'], $payment_row['supplier_code'], $selected_month, $selected_year,
            (float)$payment_row['route_distance'], (float)$payment_row['fixed_amount'], (int)$payment_row['with_fuel'], 
            $payment_row['fuel_efficiency'] ?? null, $payment_row['rate_id'] ?? null, $consumption_rates, $default_km_per_liter
        );

        $adjustment_vs_db = get_total_adjustment_amount($conn, $payment_row['route_code'], $payment_row['supplier_code'], $selected_month, $selected_year) * -1;
        
        $payment_data[] = [
            'route_code' => $payment_row['route_code'], 
            'supplier_code' => $payment_row['supplier_code'], 
            'no' => substr($payment_row['route_code'], 6, 3),
            'route' => $payment_row['route'] . " (" . $payment_row['supplier_code'] . ")", 
            'price_per_1km' => $calculation_results['effective_trip_rate'], 
            'total_working_days' => $calculation_results['total_trips'], 
            'total_distance' => ($payment_row['route_distance'] / 2) * $calculation_results['total_trips'], 
            'other_amount' => $adjustment_vs_db, 
            'payments' => $calculation_results['total_payment'] + $adjustment_vs_db
        ];
    }
    $payments_stmt->close();
}

$page_title = "Staff Monthly Payments";
$table_headers = ["No", "Route (Supplier)", "Trip Rate", "Trips", "Distance (km)", "Adjustment", "Total (LKR)", "PDF"];
...
<head>
    ...
    <style>
        ...
        /* Dropdown Menu Styles (No Hover, Click Only) */
...
<div class="bg-gradient-to-r from-gray-900 to-indigo-900 text-white h-16 flex justify-between items-center shadow-lg w-[85%] ml-[15%] px-6 sticky top-0 z-40 border-b border-gray-700">
    ...
    <div class="flex items-center gap-4 text-sm font-medium">
        ...
        <form method="get" action="payments_category.php" class="flex items-center">
            <div class="relative">
        ...
        <a href="download_route_payments.php?month=<?php echo $selected_month; ?>&year=<?php echo $selected_year; ?>" 
        ...
        <a href="payments_done.php" class="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-3 py-1.5 rounded-md shadow-md transition transform hover:scale-105 font-semibold text-xs tracking-wide">
            <i class="fas fa-check-circle"></i> Done
        </a>

        <a href="payments_history.php" class="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1.5 rounded-md shadow-md transition transform hover:scale-105 font-semibold text-xs tracking-wide">
                            ...
                            <?php foreach ($payment_data as $data): ?>
                                <tr class="hover:bg-indigo-50 transition duration-150 group">
                                    ...
                                    <td class="py-3 px-6 text-center">
                                        ...
                                        ?>
                                        <a href="download_staff2_pdf.php?route_code=<?= $data['route_code'] ?>&supplier_code=<?= $data['supplier_code'] ?>&month=<?= $selected_month ?>&year=<?= $selected_year ?>&calculated_payment=<?= $calc_pay ?>&total_distance_calc=<?= $tot_dist ?>"
