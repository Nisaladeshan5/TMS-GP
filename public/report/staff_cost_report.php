...
// --- 2. GET TOTAL STAFF COST ---

// Staff Transport Cost comes from monthly_payments_sf
$sql_cost = "SELECT SUM(monthly_payment) as total_cost FROM monthly_payments_sf WHERE year = ? AND month = ?";
...
$totalStaffCost = $row_cost['total_cost'] ?? 0;
$stmt_cost->close();


// --- 3. GET EMPLOYEE COUNTS (SMART LOGIC) ---

// A. Check History Table (monthly_staff_summary)
// අපි කලින් Staff Headcount එකට වෙනම Table එකක් හැදුවා නම් එය භාවිතා කරයි.
$sql_check = "SELECT COUNT(*) as cnt FROM monthly_staff_summary WHERE year = '$year' AND month = '$month'";
$check_res = $conn->query($sql_check);
$has_history = ($check_res && $check_res->fetch_assoc()['cnt'] > 0);
...
if ($grandTotalEmployees > 0) {
    $costPerHead = $totalStaffCost / $grandTotalEmployees;
}

// --- 5. EXCEL EXPORT HEADERS ---
$monthName = date('F', mktime(0, 0, 0, $month, 10));
$filename = 'Staff_Cost_Distribution_' . $monthName . '_' . $year . '.xls';
...
<table border="1" style="font-family: Arial, sans-serif; font-size: 12px;">
    <thead>
        <tr>
            <th colspan="6" style="background-color: #404040; color: white; font-size: 14px; text-align: center; height: 30px;">
                Staff Transport Cost Distribution - <?php echo $monthName . ' ' . $year; ?>
            </th>
        </tr>
    ...
    <tfoot>
        <tr style="font-weight: bold;">
            <td style="background-color: #cccccc; border: 1px solid #ffffff;">GRAND TOTAL</td>
            <td style="background-color: #cccccc; text-align: center; border: 1px solid #ffffff;"><?php echo $sumDirectQty; ?></td>
            <td style="background-color: #cccccc; text-align: center; border: 1px solid #ffffff;"><?php echo $sumIndirectQty; ?></td>
            <td style="background-color: #cccccc; text-align: right; border: 1px solid #ffffff;"><?php echo number_format($sumDirectCost, 2); ?></td>
            <td style="background-color: #cccccc; text-align: right; border: 1px solid #ffffff;"><?php echo number_format($sumIndirectCost, 2); ?></td>
            <td style="background-color: #cccccc; text-align: right; border: 1px solid #ffffff;"><?php echo number_format($sumTotalCost, 2); ?></td>
        </tr>
    </tfoot>
</table>
