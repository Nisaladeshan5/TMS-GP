...
<head>
    ...
    <title>Transport Scan Terminal (Factory)</title>
...
<div class="fixed top-0 left-[15%] w-[85%] bg-slate-900/90 backdrop-blur border-b border-slate-800 h-16 flex justify-between items-center px-6 shadow-2xl z-50">
    <div class="flex items-center gap-3">
        <div class="text-lg font-bold tracking-wide bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-200 bg-clip-text text-transparent">
            Scan Terminal <span class="text-xs font-normal text-slate-400 ml-1">(Factory)</span>
        </div>
    </div>
    
    <div class="flex items-center gap-4 text-sm font-medium">
        <a href="../../registers/factory_transport_vehicle_register.php" class="text-slate-400 hover:text-cyan-400 transition flex items-center gap-2">
    ...
    <div class="w-full max-w-lg bg-slate-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div class="bg-indigo-600 p-4 text-center shadow-lg">
            <h1 class="text-xl font-bold text-white flex justify-center items-center gap-2">
                <i class="fas fa-qrcode"></i> Factory Transport Scan
            </h1>
        </div>
                ...
                <div class="bg-slate-800/50 rounded-xl p-3 mb-3">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-slate-900/80 p-3 rounded-lg border border-white">
                            ...
                            <input type="text" id="editableVehicleNo" class="hidden w-full bg-transparent border-b border-cyan-500 text-white font-mono font-bold text-base focus:outline-none" placeholder="Enter No" oninput="this.value=this.value.toUpperCase();">
                        </div>
                        ...
                        <div class="bg-slate-900/80 p-3 rounded-lg  border border-white">
                            ...
                            <input type="text" id="editableDriverNIC" class="hidden w-full bg-transparent border-b border-cyan-500 text-white font-mono font-bold text-base focus:outline-none" placeholder="Enter NIC" oninput="this.value=this.value.toUpperCase();">
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function fetchTransportDetails(routeCode) {
        ...
        // Using Factory PHP file: f_get_transport_details.php
        fetch(`f_get_transport_details.php?route_code=${encodeURIComponent(routeCode)}&shift=${encodeURIComponent(getCurrentShift())}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Logic from Factory Code: Force 'IN' transaction type
                    // If the backend sends it, use it. If not, default to 'in'.
                    ...
                    detailTransactionType.textContent = "IN"; // Force Display to IN
                    
                    // Force Badge Color to Green (IN Style)
                    detailTransactionType.className = `text-sm font-black px-4 py-1 rounded shadow-lg tracking-wider bg-emerald-500 text-black`;
                    ...
                    // Ensure Unknown/Correction checkboxes are visible (as per factory logic for defaults)
    ...
    submitBtn.addEventListener('click', function() {
        ...
        if (currentScanData) {
            ...
            const transactionType = 'in'; // Hardcoded to 'in' as per Factory Logic
    ...
    function submitTransportTransaction(routeCode, vehicleNo, driverNIC, transactionType, recordId, shift, vehicleStatus, driverStatus) {
        ...
        // Using Factory PHP file: f_submit_transport_transaction.php
