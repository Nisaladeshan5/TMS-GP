...
<head>
    ...
    <title>Transport Management System - IN ONLY</title>
...
<body class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="w-[85%] ml-[15%]">
            <div class="bg-gray-800 text-white p-2 flex justify-between items-center shadow-lg w-full">
                ...
                <div class="flex gap-4 pr-4">
                    <a href="../../registers/factory_transport_vehicle_register.php" class="hover:text-yellow-400 transition">Factory Register</a>
            ...
            <div class="main-card bg-white p-6 sm:p-10 rounded-xl shadow-2xl w-full max-w-lg mx-auto mt-6">
                ...
                <h1 class="text-3xl font-extrabold mb-8 text-center text-gray-900 border-b-2 border-indigo-100 pb-3">
                    Transport Scan Terminal <span class="text-red-600">(Factory)</span>
                </h1>
                ...
                <div id="transportDetails" class="hidden bg-indigo-50 border border-indigo-200 text-indigo-900 p-4 sm:p-6 rounded-xl shadow-inner">
                    ...
                    <div class="grid grid-cols-2 gap-y-2 mb-4">
                        ...
                        <span id="detailTransactionType" class="font-bold text-lg text-green-600 uppercase text-right">IN</span>
                    </div>
                    ...
                    <div class="space-y-4 border-t border-indigo-100 pt-2 mt-2">
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                            ...
                            <div class="flex items-center w-full sm:w-2/3 space-x-4">
                                ...
                                <input type="text" id="editableVehicleNo" class="hidden border border-gray-300 rounded-lg py-2 px-3 text-gray-700 focus:ring-2 focus:ring-yellow-400 w-full" placeholder="Enter Vehicle No" oninput="this.value=this.value.toUpperCase();">
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                            ...
                            <div class="flex items-center w-full sm:w-2/3 space-x-4">
                                ...
                                <input type="text" id="editableDriverNIC" class="hidden border border-gray-300 rounded-lg py-2 px-3 text-gray-700 focus:ring-2 focus:ring-yellow-400 w-full" placeholder="Enter Driver NIC" oninput="this.value=this.value.toUpperCase();">
                                <div class="flex items-center whitespace-nowrap">
    ...
    const detailTransactionType = document.getElementById('detailTransactionType'); // Keep reference, but will be fixed to 'IN'
    ...
    function getCurrentShift() {
        ...
        // Shift logic remains the same (Morning 00:00 to 11:59, Evening 12:00 to 23:59)
    ...
    function fetchTransportDetails(routeCode) {
        resetUI();
        showMessage('Fetching transport details for arrival...', 'info');

        // Note: The backend get_transport_details.php should now only return 'IN' specific data.
        fetch(`f_get_transport_details.php?route_code=${encodeURIComponent(routeCode)}&shift=${encodeURIComponent(getCurrentShift())}`)
            .then(response => { if (!response.ok) throw new Error('Network response was not ok'); return response.json(); })
            .then(data => {
                // We now assume the transaction type is always 'in'
                ...
                if (data.success) {
                    ...
                    // detailTransactionType is now fixed to 'IN' in HTML and JS is simplified

                    // For 'in' transactions, we display defaults and allow editing
                    ...
                    // Ensure Unknown/Correction checkboxes are visible for editing defaults
                    ...
                    // existing_record_id might be relevant if you're updating a prior 'out' record, so we keep it.
    ...
    submitBtn.addEventListener('click', function() {
        ...
        if (currentScanData) {
            ...
            // The transaction type is fixed to 'in'
            const transactionType = 'in'; 
            ...
            // Status is 0 if manually entered (Unknown/Correction is checked), 1 if using default (Assigned)
    ...
    function submitTransportTransaction(routeCode, vehicleNo, driverNIC, transactionType, recordId, shift, vehicleStatus, driverStatus) {
        ...
        fetch('f_submit_transport_transaction.php', {
            method: 'POST',
            body: formData
        })
    ...
    function resetUI() {
        ...
        detailTransactionType.textContent = 'IN'; // Resetting the fixed label
        detailTransactionType.classList.remove('text-red-600','text-blue-800');
        detailTransactionType.classList.add('text-green-600'); // Ensure it remains green for IN
