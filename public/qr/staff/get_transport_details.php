...
if (isset($_GET['route_code']) && isset($_GET['shift'])) {
    ...
    if ($result_check->num_rows > 0) {
        ...
        $stmt_route = $conn->prepare("SELECT r.route, r.vehicle_no, v.driver_NIC FROM route r JOIN vehicle v ON r.vehicle_no = v.vehicle_no WHERE r.route_code = ? AND r.purpose = 'staff'");
        $stmt_route->bind_param("s", $route_code);
        $stmt_route->execute();
        $route_result = $stmt_route->get_result();
